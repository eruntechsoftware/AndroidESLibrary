		...
		try
		{
			...
			for(datareader.moveToFirst();!datareader.isAfterLast();datareader.moveToNext()){
				...
				for(String name:names){
					...
					if(!value.trim().equals("") && !ValidatorHelper.isMached("^[A-Za-z0-9]+$", value.toString()) && !ValidatorHelper.isMached("^\\s*-?\\s*\\d+(.\\d+)?\\s*$", value.toString())
							&& !ValidatorHelper.isMached("^[\u4E00-\u9FFF]+$", value.toString()))
					{
						value = new String(datareader.getBlob(columnIndex), charCode);
					}
					data.setValue(value);
