...
/**
 * 1.AndroidManifest.xml的Application节点添加 <provider android:name="android.support.v4.content.FileProvider"
 * android:authorities="com.yancy.gallerypickdemo.fileprovider" android:exported="false"
 * android:grantUriPermissions="true"> <meta-data android:name="android.support.FILE_PROVIDER_PATHS"
 * android:resource="@xml/filepaths" /> </provider> 2.values下边添加名为xml的文件夹，新建文件名为filepaths.xml 内容 <?xml version="1.0"
 * encoding="utf-8"?> <resources> <paths> <external-path name="external" path="" /> <files-path name="files" path="" />
 * <cache-path name="cache" path="" /> </paths> </resources>
 **/
...
public class ESPhotoAddView extends ESGridView implements AdapterView.OnItemClickListener, IDataInitialize, ESActionSheet.OnActionSheetClickListener, BitmapCollection.BitmapCollectionDelegate
{
	...
	public void dataInitialize()
	{
		...
		galleryConfig = new GalleryConfig.Builder().imageLoader(new FrescoImageLoader(activity))    // ImageLoader 加载框架（必填）
